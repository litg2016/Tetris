...
const int sharp[15][8] =          //组成图形的各个点的各个坐标，先纵后横 4行4列，每一对是一个点的坐标 共有4个点
{
...
const int high[15] = { 4,1,2,2,3,2,3,2,3,2,3,2,3,2,3 }; //15个图形的高度
int map[28][16];
...
void Tetris::Pause()        // 暂停函数
{
    ...
    while (1)
    {
        while (1)
        {
            if (_kbhit()) //keyboard hit  检查当前是否有键盘输入，若有则返回一个非0值，否则返回0。
            {
                temp = _getch();
                break;
            }
...
void Tetris::SetColor(int color_num)      //设置颜色
{
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), n); //控制字体颜色和背景色
}
...
void Tetris::Run()          //运行游戏
{
    ...
    while (1)
    {
        ...
        {
            i = 0;
            if (Judge(point[0] + 1, point[1], id))      //在某一位置不能下落的话 图形到底
            {
                Updata();
                id = next_id;
